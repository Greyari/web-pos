...
<body class="bg-slate-900 antialiased" x-data="{ sidebarOpen: false }">
   ...
   {{ request()->routeIs('report.product') ? 'nav-active' : 'hover:bg-white/10 text-white' }}">
                ...
                <a href="{{ route('pc-build.index') }}" class="flex items-center text-sm px-6 py-3 transition-all rounded-xl {{ request()->routeIs('pc-build') ? 'nav-active' : 'hover:bg-white/10 text-white' }}">
                    ...
                    <span class="ml-3 {{ request()->routeIs('pc-build') ? 'font-semibold' : 'font-normal' }}">Simulasi PC</span>
                </a>
