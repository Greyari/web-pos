...
@section('content')
    <div class="px-5">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-slate-500">
                Perbarui informasi produk dan supplier
            </p>
        </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm">{{ old('description', $product->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="supplier-row bg-slate-50 border border-slate-200 rounded-2xl p-5"
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div class="md:col-span-2">
                                            ...
                                            <select name="suppliers[{{ $i }}][supplier_id]"
                                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm"
                                                required>
                                                @foreach ($suppliers as $sup)
                                                    <option value="{{ $sup->id }}"
                                                        {{ $s['supplier_id'] == $sup->id ? 'selected' : '' }}>
                                                        {{ $sup->nama_supplier }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="suppliers[{{ $i }}][stock]"
                                                value="{{ $s['stock'] }}"
                                                class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm">
                                        </div>
                                        ...
                                        <div class="md:col-span-2">
                                            <div class="bg-white border border-slate-200 rounded-xl p-3 text-sm">
                                                ...
                                                <span class="margin-display font-semibold ml-1">
                                                    Rp
                                                    {{ number_format($s['harga_jual'] - $s['harga_beli'], 0, ',', '.') }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
    ...
    `;
        ...
        function calculateMargin(i) {
            ...
            display.className =
                'margin-display font-semibold ml-1 ' + (margin >= 0 ? 'text-green-600' : 'text-red-600');
        }
